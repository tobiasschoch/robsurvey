...
hub <- function(x, w = NULL, k, tol = 1e-6, maxit = 50){
   ...
   robwgt = as.double(numeric(n)), k = as.double(k), loc = as.double(numeric(1)), 
   scale = as.double(numeric(1)), n = as.integer(n), maxit = as.integer(maxit), 
   tol = as.double(tol))
   tmp$convergend <- TRUE
   if (tmp$maxit == maxit) tmp$convergend <- FALSE
   tmp
}

x <- c(200,0.55,-0.34,-0.91,0.61,0.45,-1.07,0.3,0.46,-1.22,-0.04,-1.19,-0.17,-0.31,0.99,0.35,-0.38,0.9,0.31,-1.35)
