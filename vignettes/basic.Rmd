...
We consider estimating the population one-sided trimmed *mean* of LOS. The lower end of the distribution is not trimmed (lower bound: `LB = 0`). The 5% largest observations are trimmed (upper bound: `UB = 0.95`). The range of values to be considered for estimation is thus defined as $[0, 0.95]$
...
The weight reduction method is due to [Dalén](#biblio) (1987). Consider the weighted $x$-total $\sum_{i \in s} w_i x_i$, where summation is over the set of elements in the sample $s$. The estimator of Dalén censors $w_ix_i$ to a constant, $c$, if $w_ix_i > c$.
