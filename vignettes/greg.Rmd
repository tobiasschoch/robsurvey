---
title: "Vignette: Robust Regression Estimation/ Prediction (GREG)"
...
output:
    html_document:
        ...
        highlight: tango
vignette: >
  %\VignetteIndexEntry{Robust Regression Estimation/ Prediction}
...
The estimated multivariate location and scatter or covariance matrix can be extracted from object `mv` with the functions `center()` and `vcov()`, respectively. The robust Mahalanobis distances can be extracted with
...
<p>
For the purpose of comparison, we have fitted the above model with the function <code>rlm</code> in package <code>MASS</code> ([Venables and Ripley](#biblio), 2002).
```{r}
library(MASS)
